        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mt-4">
                        <label for="filterKelas" class="block text-sm font-medium text-gray-700">Tampilkan Kelas:</label>
                        <select id="filterKelas" name="filterKelas" 
                                onchange="window.location.href = this.value;"
                                class="mt-1 block w-full md:w-1/3 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            
                            <option value="{{ route('dosen.absensi.show', ['matkul' => $matkul, 'sesi' => $sesi]) }}"
                                    {{ !$filterKelasId ? 'selected' : '' }}>
                                -- Tampilkan Semua Kelas --
                            </option>
                            
                            @foreach($daftarKelas as $kelas)
                                <option value="{{ route('dosen.absensi.show', ['matkul' => $matkul, 'sesi' => $sesi, 'kelas_id' => $kelas->id]) }}"
                                        {{ $filterKelasId == $kelas->id ? 'selected' : '' }}>
                                    {{ $kelas->nama_kelas }}
                                </option>
                            @endforeach
                        </select>
